...
app.get('/search', async (req, res) => {
  ...
  const { rows: results, totalCount } = await searchWikipedia(q, limit, offset);
  const countStart = offset + 1;
  const countEnd = offset + results.length;
  const countTotal = totalCount;

  // Render results
  res.render('index', { q, words, exact, exclude, oneof, results, countStart, countEnd, countTotal, page, limit });
});
